...
void TemplateMatching(Mat src, Mat template_image)
{
	...
	// 第一段階
	Mat src1_4 = MakePyr(src, 2);  // ４分の１の入力画像
	Mat tmp1_4 = MakePyr(template_image, 2);  // ４分の１のテンプレート画像
	...
	// 第二段階
	Mat src1_2 = MakePyr(src, 1);  // ２分の１の入力画像
	Mat tmp1_2 = MakePyr(template_image, 1);  // ２分の１のテンプレート画像
	sad_min = INT_MAX;
	...
	Mat tmp1_1 = template_image.clone();  // 元画像と同じ大きさのテンプレート画像
	sad_min = INT_MAX;
	...
	namedWindow("Rectangle", WINDOW_AUTOSIZE);  // ウィンドウ表示の準備
